...
public class CloseOrderTask {
    ...
    public void closeOrderTask() {
        ...
        if (isSuccess) {
            ...
            String lockStr = (String) redisTemplate.opsForValue().get(ServerConst.RedisLock.CLOSE_ORDER_LOCK_KEY);
            //如果这个时间戳不为null且当前时间已经大于了原来的时间，此时就表示可以去抢夺资源了
            //这也就防止了某个资源被同一个进程持续持有
            if (System.currentTimeMillis() > Long.parseLong(lockStr)) {

                //使用getSet方法来设置新值并返回旧值，如果旧值为null，表示可以获取锁了
                //如果不为null，但是旧值和lockStr相同，就说明期间没有其他线程来设置值，即没有其他线程抢先一步获取锁
                //那么，当前线程就有权利获取锁，这又防止了死锁
                String getSetResult = (String) redisTemplate.opsForValue().getAndSet(ServerConst.RedisLock.CLOSE_ORDER_LOCK_KEY,
                        String.valueOf(System.currentTimeMillis()) + timeOut);
                if (StringUtils.equals(getSetResult, lockStr)) {
                    closeOrder(coreProperties.getTask().getCloseOrderTime());
                } else {
                    log.info("获取锁失败，放弃任务");
                }
